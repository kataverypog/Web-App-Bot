...
const StreamerList = () => {
    const {bgColor, color, secondaryBG, btnColor, hintColor, linkColor} = useContext(ColorContext);
    const {user, editStreamer, handleStart, handleStop} = useContext(UserContext);
    ...
    const saveEdit = () => {
        editStreamer(editingId, editedName);
        setEditingId(null);
        setEditedName("");
    };
    ...
    return (
        <>
            <div style={{backgroundColor: bgColor, color, minHeight: "100vh", padding: "16px"}}>
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl">List of Streamers</h2>
                    <h2>{user.username}</h2>
                </div>
                <AddStreamer/>
                <div style={{display: "flex", flexDirection: "column", gap: "6px"}}>
                    {user.streamers && user.streamers.length > 0 ? (
                        user.streamers.map((s) => (
                            <div
                                key={s.id}
                                style={{
                            ...
                            >
                                ...
                                <div className="flex gap-2">
                                    {editingId === s.id ? (
                                        ...
                                        <button onClick={() => startEditing(s.id, s.name)}
                                                className="p-0.5 rounded-full"
                                                style={{backgroundColor: bgColor}}>
                                            <svg
                                                viewBox="0 -0.5 25 25"
                                    ...
                                    <DeleteStreamer id={s.id}/>
                                </div>
                            </div>
                        ))
                    ) : (
                        <p style={{color: hintColor}}>No Streamers to display</p>
                    )}
                </div>
            </div>
            <button
            ...
            >
                {user.process ? "Stop Bot" : "Start Bot"}
            </button>
        </>
    );
};
